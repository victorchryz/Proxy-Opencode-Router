...
const ENV_STREAM_TIMEOUT_MS = parseInt(process.env.PROXY_STREAM_TIMEOUT_MS, 10) || 120000;
...
function applyBackoff(state, endpoint, status, errBody, tag, headers) {
  ...
  if (isDegraded) {
    ...
    state.backoffIndex = 0;
    console.log(`${getLogTime()} 🟡 [Degradado] ${tag} DEGRADED — bloqueado por ${waitMin} min, tentando próximo.`);
    return false;
  }
...
function injectModelTag(eventStr, model, tagState, reqId, isFallback) {
...
function normalizeSSEEvent(eventStr, isKimi, kimiState) {
  ...
  if (delta) {
    ...
    if (kimiState) {
      if (delta.content && delta.content.trim() !== '') {
        ...
        kimiState.kimiContentBuf += delta.content;
      }
      if (Array.isArray(delta.tool_calls) && delta.tool_calls.length > 0) {
        kimiState.kimiEmittedToolCalls = true;
        kimiState.kimiEmittedAnswer = true;
      }
  ...
  // Rastreia o finish_reason mesmo em chunks sem delta (ex.: {"choices":[{"finish_reason":"stop"}]})
  if (kimiState && choice && choice.finish_reason) {
    kimiState.kimiFinishReason = choice.finish_reason;
  }
...
const server = http.createServer(async (req, res) => {
  ...
  const reqId = `${Date.now().toString(36).slice(-4)}-${Math.random().toString(36).slice(2, 6)}`;
  logDebug(`\n=== [OPCODE -> PROXY] [${reqId}] ${new Date().toISOString()} ===\n${bodyString}\n`);
  ...
  req.on('close', () => {
    ...
    try {
      ...
      console.log(`${getLogTime()} [${reqId}] 🧠 [Plano] ${planLog}`);
      ...
      modelLoop: for (const endpoint of orderedEndpoints) {
        ...
        if (endpoint !== orderedEndpoints[0]) {
          console.log(`${getLogTime()} [${reqId}] 🔁 [Cascata] Roteando para ${getVisualTag(endpoint.provider, endpoint.model, kIdx)}...`);
        }
        ...
        while (tentativas < maxTentativas && !requestComplete && !clientDisconnected) {
          ...
          const initialTimer = setTimeout(() => {
            console.warn(`${getLogTime()} [${reqId}] 🔴 [Timeout] ${ENV_CONN_TIMEOUT_MS}ms excedido em ${getVisualTag(endpoint.provider, endpoint.model, kIdx)}. Abortando...`);
            controller.abort();
          }, ENV_CONN_TIMEOUT_MS);
          ...
          const resetChunkTimer = (activeCtrl) => {
            ...
            chunkTimer = setTimeout(() => {
              console.warn(`${getLogTime()} [${reqId}] 🔴 [Stream] ${ENV_STREAM_TIMEOUT_MS}ms sem dados! Abortando...`);
              activeCtrl.abort();
          ...
          try {
            ...
            console.log(`${getLogTime()} [${reqId}] 🚀 [Início] -> ${getVisualTag(endpoint.provider, endpoint.model, kIdx)}`);

            const response = await fetch(url, buildFetchOptions(url, req.method, headers, finalBody, controller.signal));

            clearTimeout(initialTimer);
            console.log(`${getLogTime()} [${reqId}] 📩 [Resposta] Status ${response.status} em ${((Date.now() - startTime) / 1000).toFixed(2)}s`);
            ...
            if (response.status >= 400) {
              ...
              console.log(`${getLogTime()} [${reqId}] ⚠️ [Erro] ${response.status} em ${getVisualTag(endpoint.provider, endpoint.model, kIdx)}: ${errBody.slice(0, 150)}`);
            ...
            let kimiState = {
              kimiEmittedAnswer: false,      // true se emitiu content não-vazio OU tool_calls
              kimiEmittedToolCalls: false,   // true se emitiu ao menos 1 tool_call (informativo/telemetria)
              kimiFinishReason: null,        // último finish_reason visto no stream (informativo/telemetria)
              kimiReasoningBuf: '',
              kimiContentBuf: '',            // acumula content emitido (pra detectar fim truncado)
              kimiFinishChunkBuf: null,
              kimiStreamId: null,
              kimiNeedsFallback: false,
              kimiFallbackReason: null,      // 'silencio' (A) | 'acao_anunciada' (B) | 'stream_cortado'
            };
            ...
            try {
              for await (let chunk of response.body) {
                ...
                for (let eventStr of events) {
                  ...
                  const { eventStr: taggedStr, tags } = injectModelTag(eventStr, endpoint.model, tagState, reqId, false);
              ...
              console.warn(`${getLogTime()} [${reqId}] ⚠️ [Stream] Cortado: ${streamErr.message}`);
            } finally {
              clearTimeout(chunkTimer);
              if (!clientDisconnected) {
                // Fallback automático para todos os modelos. Padrões detectados:
                // A) Silêncio total: só reasoning, sem content nem tool_calls
                // B) Ação anunciada: emitiu content terminando em :/?/?/... mas não chamou tool
                // C) Stream cortado: stream terminou sem nunca receber finish_reason
              if (!kimiState.kimiEmittedAnswer) {
                // Padrão A — Silêncio total: só reasoning, sem content nem tool_calls
                kimiState.kimiNeedsFallback = true;
                kimiState.kimiFallbackReason = 'silencio';
                console.log(`${getLogTime()} [${reqId}] 🔄 [${endpoint.name}] Padrão A (silêncio): só reasoning, sem resposta. Fallback.`);
              } else if (!kimiState.kimiEmittedToolCalls) {
                // Padrão B — Emitiu content mas não chamou tool. Verifica se o texto
                // parece anunciar uma ação (termina em :, ... ou ?) → prometeu mas não fez.
                const contentTail = (kimiState.kimiContentBuf || '').trim();
                const looksTruncated = /[:：…‹]|\.{3,}$|(\?|？)$/.test(contentTail);
                if (looksTruncated) {
                  kimiState.kimiNeedsFallback = true;
                  kimiState.kimiFallbackReason = 'acao_anunciada';
                  console.log(`${getLogTime()} [${reqId}] 🔄 [${endpoint.name}] Padrão B (ação anunciada): terminou com "${contentTail.slice(-20)}" sem tool_calls. Fallback.`);
                }
              }

              // Stream cortado sem finish_reason — modelo começou a responder mas
                // o stream foi interrompido (erro/timeout) sem nunca enviar finish_reason
              if (!kimiState.kimiNeedsFallback && !kimiState.kimiFinishReason && (kimiState.kimiContentBuf || kimiState.kimiReasoningBuf)) {
                kimiState.kimiNeedsFallback = true;
                kimiState.kimiFallbackReason = 'stream_cortado';
                console.log(`${getLogTime()} [${reqId}] 🔄 [${endpoint.name}] Stream cortado sem finish_reason. Fallback.`);
              }
              }
            }
            ...
            if (kimiState.kimiNeedsFallback && !clientDisconnected) {
              ...
              logDebug(`\n=== [FALLBACK DISPARADO] [${reqId}] ${new Date().toISOString()} ===`);
              logDebug(`Modelo origem: ${endpoint.model} | Motivo: ${kimiState.kimiFallbackReason} | reasoning_buf=${kimiState.kimiReasoningBuf.length} chars`);
              ...
              for (const nextEp of fallbackCandidates) {
                ...
                const fbInitialTimer = setTimeout(() => {
                  console.warn(`${getLogTime()} [${reqId}] 🔴 [Fallback Timeout] ${ENV_CONN_TIMEOUT_MS}ms excedido em ${getVisualTag(nextEp.provider, nextEp.model, fkIdx)}. Abortando...`);
                  fbController.abort();
                }, ENV_CONN_TIMEOUT_MS);
                ...
                if (lastUserIdx >= 0) {
                  let fallbackCtx = `\n\n[Fallback - ${endpoint.model} não respondeu, tentando ${nextEp.model}]\n`;
                  if (kimiState.kimiReasoningBuf.trim().length > 0) {
                    fallbackCtx += `\nRaciocínio anterior (${endpoint.model}):\n` + kimiState.kimiReasoningBuf.trim();
                  }
                  if (kimiState.kimiContentBuf.trim().length > 0) {
                    fallbackCtx += `\n\nResposta parcial (${endpoint.model}):\n` + kimiState.kimiContentBuf.trim();
                  }
                  const msg = fbBodyParsed.messages[lastUserIdx];
                  if (Array.isArray(msg.content)) {
                    const textPart = msg.content.find(p => p.type === 'text');
                    if (textPart) textPart.text += fallbackCtx;
                    else msg.content.push({ type: 'text', text: fallbackCtx });
                  } else if (typeof msg.content === 'string') {
                    msg.content += fallbackCtx;
                  }
                }
                ...
                console.log(`${getLogTime()} [${reqId}] 🔁 [Fallback] -> ${getVisualTag(nextEp.provider, nextEp.model, fkIdx)}`);
                logDebug(`=== [PROXY -> NVIDIA FALLBACK] -> ${nextEp.model} (K${fkIdx + 1}) ===`);
                logDebug(`[PROXY -> NVIDIA] ${fbBody}`);
                logDebug(`=== [NVIDIA FALLBACK -> PROXY] ${new Date().toISOString()} ===`);
                  ...
                  try {
                    for await (let fbChunk of fbResponse.body) {
                      ...
                      for (let fbEventStr of fbEvents) {
                        ...
                        const { eventStr: fbTaggedStr } = injectModelTag(fbEventStr, nextEp.model, tagState, reqId, true);
                        if (kimiState.kimiStreamId) {
                          fbEventStr = fbTaggedStr.replace(/"id"\s*:\s*"[^"]*"/g, '"id":"' + kimiState.kimiStreamId + '"');
                        }
                        const outFb = kimiState.kimiStreamId ? fbEventStr : fbTaggedStr;
                        logDebug(`[NVIDIA -> PROXY] ${outFb}`);
                        await writeSSE(res, outFb + '\n\n');
                      }
                    }
                    if (fbSseBuffer.trim()) {
                      fbSseBuffer = normalizeSSEEvent(fbSseBuffer, fbIsKimi, kimiState);
                      const { eventStr: fbTaggedStr } = injectModelTag(fbSseBuffer, nextEp.model, tagState, reqId, true);
                      if (kimiState.kimiStreamId) {
                        fbSseBuffer = fbTaggedStr.replace(/"id"\s*:\s*"[^"]*"/g, '"id":"' + kimiState.kimiStreamId + '"');
                      }
                      const outFbTail = kimiState.kimiStreamId ? fbSseBuffer : fbTaggedStr;
                      logDebug(`[NVIDIA -> PROXY] ${outFbTail}`);
                      await writeSSE(res, outFbTail + '\n\n');
                    }
                  } catch (fbStreamErr) {
                    console.warn(`${getLogTime()} [${reqId}] ⚠️ [Fallback Stream] Cortado: ${fbStreamErr.message}`);
                    recordRequest(nextEp.model, Date.now() - requestStartTime, true, true);
                    continue;
                  }
                  recordRequest(nextEp.model, Date.now() - requestStartTime, true, false);
                  fallbackSuccess = true;
                  logDebug(`=== [FALLBACK OK] ${nextEp.model} respondeu com sucesso ===`);
                  break;

                } catch (fbFetchErr) {
                  clearTimeout(fbInitialTimer);
                  if (fbFetchErr.name === 'AbortError') {
                    console.warn(`${getLogTime()} [${reqId}] ⏱️ [Fallback Abortado] Timeout em ${getVisualTag(nextEp.provider, nextEp.model, fkIdx)}. Tentando próximo...`);
                  } else {
                    console.error(`${getLogTime()} [${reqId}] 💥 [Fallback Fetch] Erro em ${getVisualTag(nextEp.provider, nextEp.model, fkIdx)}:`, fbFetchErr.message);
                  }
                  recordRequest(nextEp.model, Date.now() - requestStartTime, true, true);
                  continue;
                }
              ...
              if (!fallbackSuccess && !clientDisconnected) {
                console.log(`${getLogTime()} [${reqId}] 🚫 [Fallback] Todos os modelos falharam`);
            ...
            if (fetchErr.name === 'AbortError') {
              if (!clientDisconnected) console.warn(`${getLogTime()} [${reqId}] ⏱️ [Abortado] Timeout em ${getVisualTag(endpoint.provider, endpoint.model, kIdx)}.`);
            } else {
              console.error(`${getLogTime()} [${reqId}] 💥 [Rede] Erro:`, fetchErr.message);
            }
            attemptsLog.push(`\x1b[31m💥\x1b[0m ${getVisualTag(endpoint.provider, endpoint.model, kIdx)}`);
            recordRequest(endpoint.model, Date.now() - startTime, false, true);
      ...
      if (!clientDisconnected) {
        console.log(`${getLogTime()} [${reqId}] 📊 [Resumo] Rota: ${attemptsLog.join(' -> ')} | Tempo: ${totalTime}s`);
      }
      ...
      console.error(`${getLogTime()} [${reqId}] 💥 [Crítico] Erro interno:`, err.message);
